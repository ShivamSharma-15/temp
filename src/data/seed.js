...
import novemberRaw from "./Asadi/inverter wms and meter data/november.json?raw";
import unitsRaw from "./Asadi/units.json?raw";
import siteOutputActivePowerRaw from "./Asadi/inverter wms and meter data/siteOutputActivePower.json?raw";
import solarIrradiationRaw from "./Asadi/inverter wms and meter data/solarIrradiation.json?raw";
import dailyEnergyRaw from "./Asadi/inverter wms and meter data/dailyEnergy.json?raw";
import jodHistoricAggregateRaw from "./JOD01/historicAggregate.json?raw";
...
const ASADI_SITE_KEY = "6a11fc90ac";
const JOD_SITE_KEY = "6b809f8b8b";
      ...
      const normalizeRow = (rowArray) => {
        ...
        for (let index = 0; index < rowArray.length; index += 1) {
          ...
          if (
            ...
            item.Data.toLowerCase().includes("sungrow")
          ) {
            const capacityEntry =
              rowArray[index + 1]?.Data === "DC Capacity 1"
                ? rowArray[index + 1]
                : null;
            const generationPerKwEntry = rowArray[
              index + 2
            ]?.Data?.toLowerCase?.().includes("generation per kw")
              ? rowArray[index + 2]
              : null;
            entries.push({
              unitName: item.Data,
...
const JOD_HISTORIC_AGGREGATE = (() => {
  try {
    const parsed = JSON.parse(jodHistoricAggregateRaw);
    return Array.isArray(parsed) ? parsed : [];
  } catch (error) {
    console.warn("Unable to parse JOD01 historicAggregate.json", error);
    return [];
  }
})();
const UNITS_CATALOG = (() => {
...
const OUTPUT_ACTIVE_POWER_SERIES = parseIntradayArray(
  siteOutputActivePowerRaw,
  "siteOutputActivePower"
);
const SOLAR_IRRADIATION_SERIES = parseIntradayArray(
  solarIrradiationRaw,
  "solarIrradiation"
);
const DAILY_ENERGY_SERIES = parseIntradayArray(dailyEnergyRaw, "dailyEnergy");

const combineIntradaySeries = (
  powerSeries,
  irradiationSeries,
  energySeries,
  prSeries = [],
...
const extractIsoDateFromPath = (path) => {
  const match = path.match(/\.\/[^/]+\/([^/]+)\//);
  if (!match) return null;
  return toIsoDateFromFolder(match[1]);
};

const loadIntradayByDateFromFiles = (
  powerFiles,
  irradiationFiles,
  energyFiles,
  prFiles = {},
  ...
  const assignSeries = (files, key) => {
    Object.entries(files).forEach(([path, data]) => {
      const isoDate = extractIsoDateFromPath(path);
      if (!isoDate) return;
...
const loadCardsByDateFromFiles = (...cardFileMaps) => {
  const byDate = {};
  const assignCards = (files) => {
    Object.entries(files).forEach(([path, data]) => {
      const isoDate = extractIsoDateFromPath(path);
      if (!isoDate) return;
  ...
  cardFileMaps.forEach((files) => assignCards(files));

  const dates = Object.keys(byDate).sort();
  return { byDate, dates };
};

const ASADI_INTRADAY = loadIntradayByDateFromFiles(
  import.meta.glob("./Asadi/*/siteOutputActivePower.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./Asadi/*/solarIrradiation.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./Asadi/*/dailyEnergy.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./Asadi/*/Pr.json", {
    eager: true,
    import: "default",
  }),
  3402.98
...
const JOD_INTRADAY = loadIntradayByDateFromFiles(
  import.meta.glob("./JOD01/*/siteOutputActivePower.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./JOD01/*/solarIrradiation.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./JOD01/*/dailyEnergy.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./JOD01/*/Pr.json", {
    eager: true,
    import: "default",
  }),
  3357.2
...
const ASADI_CARDS = loadCardsByDateFromFiles(
  import.meta.glob("./Asadi/*/cardsData.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./Asadi/*/cardData.json", {
    eager: true,
    import: "default",
  })
);

const JOD_CARDS = loadCardsByDateFromFiles(
  import.meta.glob("./JOD01/*/cardsData.json", {
    eager: true,
    import: "default",
  }),
  import.meta.glob("./JOD01/*/cardData.json", {
    eager: true,
    import: "default",
  })
);

const AVAILABLE_INTRADAY_DATES = Array.from(
  new Set([...(ASADI_INTRADAY.dates ?? []), ...(JOD_INTRADAY.dates ?? [])])
).sort();
const INTRADAY_BY_DATE = {
  ...ASADI_INTRADAY.byDate,
  ...JOD_INTRADAY.byDate,
};

const HARD_MAX_DATE =
  AVAILABLE_INTRADAY_DATES.at(-1) ?? "2025-11-25";
...
const ASADI_LAST_DAY_SERIES =
  ASADI_INTRADAY.dates.length > 0
    ? ASADI_INTRADAY.byDate[ASADI_INTRADAY.dates.at(-1)] ?? []
...
const buildAsadiEnergySeries = (dailyBuckets, capacityMw) => {
  ...
  return dailyBuckets.map(({ date, entries }) => {
    ...
    const avgGenerationPerKw = genPerKwValues.length
      ? +(
          genPerKwValues.reduce((sum, value) => sum + value, 0) /
          genPerKwValues.length
        ).toFixed(2)
      : null;
...
const buildAsadiUnits = (meta, latestEntries, capacityMw) => {
  const siteKey = meta.site_key ?? ASADI_SITE_KEY;
  const catalogUnits = UNITS_CATALOG.filter(
    (unit) => unit.site_key === siteKey
  );
  const deriveType = (category, name = "") => {
    const lower = name.toLowerCase();
    if (category?.toLowerCase() === "inverter" || lower.includes("sungrow"))
  ...
  if (catalogUnits.length) {
    const inverterCount = catalogUnits.filter(
      (unit) =>
        deriveType(unit.unit_category_name, unit.name) === "Central Inverter"
    ).length;
  ...
  return entries.map((entry, index) => {
    const ratedMw =
      toNumber(
        entry["DC Capacity 1"],
        capacityMw ? (capacityMw * 1000) / entries.length : 0
      ) / 1000;
    ...
    return {
      ...
      ratedKw: +(
        ratedKw || (capacityMw / Math.max(1, entries.length)) * 1000
      ).toFixed(1),
      ratedMw: +(ratedMw || capacityMw / Math.max(1, entries.length)).toFixed(
        2
      ),
      status: "Online",
      lastOutputKwh,
...
const buildAsadiLastDaySeries = (dailyBuckets) => {
  ...
  const baseTimestamp = Math.floor(
    new Date(`${latest.date}T06:00:00`).getTime() / 1000
  );
...
const buildJodEnergySeries = (aggregates, capacityMw, fieldMeta) => {
  const sorted = Array.isArray(aggregates)
    ? [...aggregates].sort((a, b) => (a.date ?? "").localeCompare(b.date ?? ""))
    : [];
  if (!sorted.length) return [];

  const totalDcFallbackKw = +(Math.max(0, capacityMw) * 1000 || 0).toFixed(2);

  const meterFieldLabels = {
    ac_active_export_energy_kwh: "AC Active Export (kWh)",
    main_net_export_kwh: "Main Net Export (kWh)",
    ac_active_import_energy_kwh: "AC Active Import (kWh)",
    net_import_kwh: "Net Import (kWh)",
    net_generation_kwh: "Net Generation (kWh)",
    ac_reactive_export_energy_kvarh: "AC Reactive Export (kVArh)",
    net_export_kvarh: "Net Export (kVArh)",
    ac_reactive_import_energy_kvarh: "AC Reactive Import (kVArh)",
    net_import_kvarh: "Net Import (kVArh)",
  };

  const inverterFieldLabels = {
    generation_kwh: "Generation (kWh)",
    dc_capacity_kw: "DC Capacity (kW)",
    generation_per_kw: "Generation per kW",
  };

  const seenKeys = fieldMeta ?? { meter: new Map(), inverter: new Map() };

  return sorted.map((entry) => {
    const inverters = Array.isArray(entry.inverters) ? entry.inverters : [];
    const totalKwh = toNumber(entry.total_generation_kwh, 0);
    const totalDcKw =
      inverters.reduce(
        (sum, inverter) => sum + toNumber(inverter.dc_capacity_kw, 0),
        0
      ) || totalDcFallbackKw;

    const perKwValues = inverters
      .map((inverter) => toNumber(inverter.generation_per_kw, NaN))
      .filter((value) => Number.isFinite(value));
    const avgGenerationPerKw =
      perKwValues.length > 0
        ? +(
            perKwValues.reduce((sum, value) => sum + value, 0) /
            perKwValues.length
          ).toFixed(2)
        : totalDcKw
        ? +((totalKwh ?? 0) / totalDcKw).toFixed(2)
        : null;

    const cufPercent = toNumber(entry.performance?.cuf_percent, null);
    const prPercent = toNumber(entry.performance?.pr_percent, null);
    const irradiance = toNumber(
      entry.weather?.solar_irradiation_wm2,
      0
    );
    const peakPowerMw = +(
      (toNumber(entry.total_generation_kwh, 0) / 24) / 1000
    ).toFixed(2);
    ...
    const record = {
      ...
      totalGenerationKwh: +totalKwh.toFixed(1),
      totalDcCapacityMw: +(totalDcKw / 1000).toFixed(3),
      totalDcCapacityKw: +totalDcKw.toFixed(2),
      avgGenerationPerKw,
      peakPowerMw,
      availabilityPct: cufPercent ?? null,
      performanceRatioPct: prPercent ?? null,
      irradianceWhm2: irradiance,
      cufPercent,
    };

    const meters =
      entry.meters && typeof entry.meters === "object" ? entry.meters : {};
    Object.entries(meters).forEach(([meterName, meterData]) => {
      if (!meterData || typeof meterData !== "object") return;
      const baseKey = meterName
        .toString()
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_");
      Object.entries(meterData).forEach(([prop, value]) => {
        const key = `${baseKey}_${prop}`;
        record[key] = toNumber(value, 0);
        if (!seenKeys.meter.has(key)) {
          const propLabel =
            meterFieldLabels[prop] ?? prettyLabel(prop.replace(/_/g, " "));
          seenKeys.meter.set(key, `${meterName} - ${propLabel}`);
        }
      });
    });
    ...
    inverters.forEach((inverter, index) => {
      const id =
        inverter.id ??
        inverter.name?.toString().match(/\d+/)?.[0] ??
        index + 1;
      const baseKey = `inv_${id}`;
      Object.entries(inverterFieldLabels).forEach(([prop, label]) => {
        const key = `${baseKey}_${prop}`;
        record[key] = toNumber(inverter[prop], 0);
        if (!seenKeys.inverter.has(key)) {
          seenKeys.inverter.set(
            key,
            `${inverter.name ?? `Inverter ${id}`} - ${label}`
          );
        }
      });
    });
    ...
    record.ambientTemperatureC = toNumber(weather.ambient_temperature_c, null);
    record.moduleTemperatureC = toNumber(weather.module_temperature_c, null);
    record.windDirectionDeg = toNumber(weather.wind_direction_deg, null);
    record.windSpeedMs = toNumber(weather.wind_speed_ms, null);

    return record;
  });
};

const buildJodCardsFromAggregate = (aggregates) => {
  const byDate = {};
  aggregates.forEach((entry) => {
    if (!entry?.date) return;
    const meters = entry.meters && typeof entry.meters === "object" ? entry.meters : {};
    const mainMeter = meters["11 Main"] ?? Object.values(meters)[0];
    if (!mainMeter) return;
    const netImport = toNumber(mainMeter.net_import_kwh, null);
    const netExport = toNumber(
      mainMeter.main_net_export_kwh ?? mainMeter.net_export_kwh,
      null
    );
    byDate[entry.date] = {
      "Net Import (kWh)": Number.isFinite(netImport) ? +netImport.toFixed(1) : null,
      "Net Export (kWh)": Number.isFinite(netExport) ? +netExport.toFixed(1) : null,
    };
  });
  return byDate;
};

const buildJodUnits = (aggregates, capacityMw) => {
  const latest = Array.isArray(aggregates) ? aggregates.at(-1) ?? {} : {};
  const inverters = Array.isArray(latest.inverters) ? latest.inverters : [];
  const inverterUnits = inverters.map((inverter, index) => {
    const ratedKw = toNumber(
      inverter.dc_capacity_kw,
      (capacityMw * 1000) / Math.max(1, inverters.length)
    );
    const lastOutputKwh = toNumber(inverter.generation_kwh, 0);
    return {
      id: inverter.name ?? `INV-${index + 1}`,
      type: "Central Inverter",
      vendor: deriveVendor(inverter.name ?? ""),
      ratedKw: +ratedKw.toFixed(1),
      ratedMw: +(ratedKw / 1000).toFixed(3),
      status: "Online",
      lastOutputKwh: +lastOutputKwh.toFixed(1),
      lastOutputMwh: +(lastOutputKwh / 1000).toFixed(3),
      temperatureC: +(42 - index * 0.6).toFixed(1),
      issues: 0,
    };
  });

  const meterEntries =
    latest.meters && typeof latest.meters === "object"
      ? Object.entries(latest.meters)
      : [];
  const meterUnits = meterEntries.map(([name, meter], index) => {
    const lastOutputKwh = toNumber(
      meter.net_generation_kwh ??
        meter.main_net_export_kwh ??
        meter.ac_active_export_energy_kwh,
      0
    );
    const ratedKw = +(capacityMw * 50).toFixed(1);
    return {
      id: name ?? `MTR-${index + 1}`,
      type: "Energy Meter",
      vendor: deriveVendor(name ?? "Meter"),
      ratedKw,
      ratedMw: +(ratedKw / 1000).toFixed(3),
      status: "Online",
      lastOutputKwh: +lastOutputKwh.toFixed(1),
      lastOutputMwh: +(lastOutputKwh / 1000).toFixed(3),
      temperatureC: +(34 + index * 0.5).toFixed(1),
      issues: 0,
    };
  });

  const units = [...inverterUnits, ...meterUnits];
  if (latest.weather) {
    units.push({
      id: "Weather Station",
      ...
      lastOutputKwh: 0,
      lastOutputMwh: 0,
      temperatureC: toNumber(latest.weather.ambient_temperature_c, 32),
      issues: 0,
    });
  }
  ...
  return [
    {
      ...
      vendor: "Sungrow",
      ratedKw: +(capacityMw * 1000).toFixed(1),
      ratedMw: +capacityMw.toFixed(3),
      status: "Online",
      lastOutputKwh: +(capacityMw * 1000 * 0.2).toFixed(1),
      lastOutputMwh: +(capacityMw * 0.2).toFixed(3),
      temperatureC: 42,
      issues: 0,
    },
  ];
...
const parseTelemetryFile = (raw) => {
  ...
  for (let index = 0; index < width; index += 1) {
    ...
    const type =
      label === "Date" ? "date" : label === "Time" ? "time" : "metric";
    let deviceName = null;
    ...
    if (inverterMatch) {
      ...
      const capacityMatch = label.match(
        /^(?:DC Capacity|Generation Per kW)\s*(\d+)/i
      );
    ...
    .map((values) => {
      ...
      columns.forEach((column) => {
        ...
        if (column.isNumeric) {
          const numeric = parseNumeric(rawValue);
          record[column.key] =
            numeric !== undefined ? numeric : column.average ?? 0;
        } else {
          record[column.key] = rawValue || null;
        }
      });
...
const buildWeatherSnapshot = (row, labelKeyMap) => {
  ...
  const module = toNumber(
    row[labelKeyMap.get("Module Temperature")],
    ambient + 5
  );
  ...
  const humidityPct = Math.max(
    25,
    Math.min(90, Math.round(78 - (module - ambient)))
  );
...
const buildHistoryFields = (columns) => {
  ...
  columns.forEach((column) => {
    if (!column.label || column.label === "Date" || column.label === "Time")
      return;
    const label =
    ...
    if (
      normalizedLabel.includes("cuf") ||
      normalizedLabel.includes("availab")
    ) {
      return;
    }
    pushUnique({
...
const buildUnitsFromTelemetry = (columns, rows, capacityMw) => {
  ...
  return [...deviceColumnMap.entries()].map(([deviceName, deviceColumns]) => {
    ...
    const ratedColumn = deviceColumns.find((column) =>
      column.label.startsWith("DC Capacity")
    );
    ...
    const energyColumn =
      deviceColumns.find((column) => column.label === deviceName) ??
      deviceColumns[0];
    const lastOutput =
    ...
    return {
      ...
      temperatureC:
        type === "Central Inverter" ? 45 : type === "Weather Station" ? 32 : 34,
      issues: 0,
    };
  });
};
...
const buildAlarmsFromSeries = (energySeries, units) => {
  ...
  const averageEnergy =
    energySeries.reduce((sum, entry) => sum + entry.energyMWh, 0) /
    energySeries.length;
  ...
  return selected.map((entry, index) => ({
    ...
    message: `Generation dipped to ${entry.energyMWh.toFixed(
      2
    )} MWh (PR ${Number(entry.performanceRatioPct ?? 0).toFixed(1)} %).`,
...
const buildSyntheticEnergySeries = (rng, capacityMw) => {
  ...
  return Array.from({ length: HISTORY_DAYS }, (_, index) => {
    ...
    return {
      date: dateISO(offset),
      energyMWh: +(
        ((theoreticalDaily * availability) / 100) *
        (0.85 + rng() * 0.2)
      ).toFixed(2),
...
const buildSyntheticSite = (meta) => {
  ...
  const capacityMw = Math.max(
    0.1,
    +(toNumber(meta.site_capacity, 0) / 1000 || 0.5).toFixed(3)
  );
  const energySeries = buildSyntheticEnergySeries(rng, capacityMw);
  const avgAvailability =
    energySeries.reduce((sum, entry) => sum + entry.availabilityPct, 0) /
    energySeries.length;
  const avgPerformance =
    energySeries.reduce((sum, entry) => sum + entry.performanceRatioPct, 0) /
    energySeries.length;
  ...
  const coordinates =
    ...
    meta.coordinates.every((value) => value?.trim?.())
      ? meta.coordinates
      : null;
  ...
  return {
    ...
    technology: ["Mono PERC modules", "Central inverters", "SCADA monitoring"],
...
const buildAsadiSite = (meta, selectedDate = DEFAULT_SELECTED_DATE) => {
  ...
  const avgPerformance =
    ...
    energySeries.length;
  const latestEntry = energySeries.at(-1);
  const units = buildAsadiUnits(
    meta,
    ASADI_DAILY_BUCKETS.at(-1)?.entries ?? [],
    capacityMw
  );
  const alarms = buildAlarmsFromSeries(energySeries, units);
  const effectiveDate = pickAvailableDate(
    selectedDate,
    ASADI_INTRADAY.dates
  );
  const lastDayData =
    ASADI_INTRADAY.dates.length > 0
      ? ASADI_INTRADAY.byDate[effectiveDate] ?? []
  ...
  const safeDayLabel = (dateString, fallbackIndex) => {
    ...
    return (
      ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][fallbackIndex % 7] ||
      "Day"
    );
  };
  ...
  const historyFields = [
    ...
    {
      key: "totalDcCapacityKw",
      label: "Total DC Capacity (kW)",
      precision: 1,
      canHide: true,
    },
    {
      key: "avgGenerationPerKw",
      label: "Avg Generation per kW",
      precision: 2,
      canHide: true,
    },
  ];
  ...
  return {
    id: siteId,
    siteKey: meta.site_key ?? ASADI_SITE_KEY,
    name: siteName,
    location,
    ...
    weather: {
      current: {
        ...
        ghi: Math.round(latestEntry?.irradianceWhm2 ?? 0),
        condition:
          latestEntry?.irradianceWhm2 > 550 ? "Sunny" : "Partly cloudy",
      },
      forecast: energySeries.slice(-3).map((entry, index) => ({
        day: safeDayLabel(entry.date, index),
        conditions:
          entry.irradianceWhm2 > 600
            ? "Sunny"
            : entry.irradianceWhm2 > 350
            ? "Cloudy"
            : "Overcast",
        temperatureC: Math.round(
          30 + (entry.performanceRatioPct - avgPerformance) * 0.1 - index
        ),
      })),
    },
    units,
    ...
    lastDayData,
    intradayByDate: ASADI_INTRADAY.byDate,
    availableIntradayDates: ASADI_INTRADAY.dates,
    cardsByDate: ASADI_CARDS.byDate,
    availableCardDates: ASADI_CARDS.dates,
    sharedAccess: [],
  };
};

const buildJodSite = (meta, selectedDate = DEFAULT_SELECTED_DATE) => {
  const siteName = meta.name ?? "JOD01";
  const siteId = `site-${slugify(meta.site_key ?? siteName)}`;
  const rawCapacityMw = toNumber(meta.site_capacity, 0) / 1000;
  const capacityMw = rawCapacityMw > 0 ? +rawCapacityMw.toFixed(3) : 0;
  const jodFieldMeta = { meter: new Map(), inverter: new Map() };
  const energySeries = buildJodEnergySeries(
    JOD_HISTORIC_AGGREGATE,
    capacityMw,
    jodFieldMeta
  );
  const aggregateCards = buildJodCardsFromAggregate(JOD_HISTORIC_AGGREGATE);
  ...
  const avgAvailability =
    energySeries.reduce(
      (sum, entry) => sum + (entry.availabilityPct ?? 0),
      0
    ) / energySeries.length;
  const avgPerformance =
    energySeries.reduce(
      (sum, entry) => sum + (entry.performanceRatioPct ?? 0),
      0
    ) / energySeries.length;
  const latestEntry = energySeries.at(-1);
  const latestAggregate = JOD_HISTORIC_AGGREGATE.at(-1) ?? {};
  const units = buildJodUnits(JOD_HISTORIC_AGGREGATE, capacityMw);
  const alarms = buildAlarmsFromSeries(energySeries, units);
  const effectiveDate = pickAvailableDate(selectedDate, JOD_INTRADAY.dates);
  const lastDayData =
    JOD_INTRADAY.dates.length > 0
      ? JOD_INTRADAY.byDate[effectiveDate] ?? []
      : [];
  const cardDates = new Set([
    ...Object.keys(aggregateCards),
    ...(JOD_CARDS.dates ?? []),
  ]);
  const cardsByDate = {};
  cardDates.forEach((date) => {
    cardsByDate[date] = {
      ...(JOD_CARDS.byDate?.[date] ?? {}),
      ...(aggregateCards[date] ?? {}),
    };
  });
  ...
  const historyFields = [
    ...
    ...Array.from(jodFieldMeta.meter.entries()).map(([key, label]) => ({
      key,
      label,
      precision: 2,
      canHide: true,
    })),
    ...Array.from(jodFieldMeta.inverter.entries()).map(([key, label]) => ({
      key,
      label,
      precision: key.includes("generation_per_kw") ? 2 : 1,
      canHide: true,
    })),
  ];
  ...
  return {
    id: siteId,
    siteKey: meta.site_key ?? JOD_SITE_KEY,
    name: siteName,
    location,
    capacityMw: capacityMw ? +capacityMw.toFixed(2) : 0,
    status: mapSiteStatus(meta.status),
    ...
    weather: {
      current: {
        temperatureC: toNumber(
          latestAggregate?.weather?.ambient_temperature_c,
          32
        ),
        humidityPct: Math.round(
          Math.min(
            ...
            Math.max(
              30,
              70 -
                toNumber(latestAggregate?.weather?.module_temperature_c, 0) *
                  0.6
            )
          )
        ),
        windKph: +(
          toNumber(latestAggregate?.weather?.wind_speed_ms, 0) * 3.6
        ).toFixed(1),
        ghi: Math.round(
          toNumber(latestAggregate?.weather?.solar_irradiation_wm2, 0)
        ),
        condition:
          toNumber(latestAggregate?.weather?.solar_irradiation_wm2, 0) > 10
            ? "Sunny"
            : "Overcast",
      },
      forecast: energySeries.slice(-3).map((entry, index) => ({
        day: safeDayLabel(entry.date, index),
        conditions:
          (entry.irradianceWhm2 ?? 0) > 600
            ? "Sunny"
            : (entry.irradianceWhm2 ?? 0) > 350
            ? "Cloudy"
            : "Overcast",
        temperatureC: Math.round(
          toNumber(
            ...
            index +
            (entry.performanceRatioPct ?? avgPerformance) * 0.02
        ),
      })),
    },
    units,
    ...
    selectedIntradayDate: effectiveDate,
    lastDayData,
    intradayByDate: JOD_INTRADAY.byDate,
    availableIntradayDates: JOD_INTRADAY.dates,
    cardsByDate,
    availableCardDates: [...cardDates].sort(),
    sharedAccess: [],
  };
};

const siteCatalogEntriesRaw = Array.isArray(siteCatalog?.sites)
  ? siteCatalog.sites
  : siteCatalog;
const siteCatalogEntries = Array.isArray(siteCatalogEntriesRaw)
  ? siteCatalogEntriesRaw
  : [];
const siteBuilders = {
  [ASADI_SITE_KEY]: (site) => buildAsadiSite(site, DEFAULT_SELECTED_DATE),
  [JOD_SITE_KEY]: (site) => buildJodSite(site, DEFAULT_SELECTED_DATE),
};

const ORDERED_SITE_KEYS = [JOD_SITE_KEY, ASADI_SITE_KEY];

const orderedCatalog = ORDERED_SITE_KEYS.map((key) =>
  siteCatalogEntries.find((site) => site.site_key === key)
).filter(Boolean);

const seedSites = orderedCatalog.map((site) => {
  const builder = siteBuilders[site.site_key];
  return builder ? builder(site) : buildSyntheticSite(site);
});
...
export const seedUsers = [
  ...
  },
  {
    ...
    role: "admin",
    accessibleSiteIds: allSiteIds.slice(
      0,
      Math.max(3, Math.ceil(allSiteIds.length / 2))
    ),
  },
  {
    id: "solar91-analyst",
    ...
    accessibleSiteIds: allSiteIds.slice(
      0,
      Math.max(1, Math.ceil(allSiteIds.length / 4))
    ),
  },
];
...
export const fleetBenchmarks = {
  ...
  averagePerformanceRatio:
    seedSites.length > 0
      ? (
          seedSites.reduce((acc, site) => acc + site.performanceRatioPct, 0) /
          seedSites.length
        ).toFixed(1)
      : "0.0",
